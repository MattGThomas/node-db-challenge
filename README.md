...
- [ ] Explain the difference between `Relational Databases` and `SQL`.
  ...
  sql is a language used to perform queries on a database
  relational databases are a type of database

- [ ] Why do tables need a `primary key`?

  identification because easier when each item in table has its own unique identifier
